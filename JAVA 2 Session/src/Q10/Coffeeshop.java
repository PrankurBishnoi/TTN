...
public class Coffeeshop implements PendingOrderQueue,CompleteOrderQueue
{
    static final PendingOrderQueue poq = new Coffeeshop();
    static final CompleteOrderQueue coq = new Coffeeshop();
    private Coffeeshop()
    {
  ...
  //static  PendingOrderQueue poq;
    //static CompleteOrderQueue coq;

    public static void main(String[] args)
    {
        Scanner s= new Scanner(System.in);
        List<Order> temp = new ArrayList<Order>();
        Customer newcust = new Customer("Shivam",temp);
        char ch;
        while(true)
        {
            System.out.println("Hi !"+newcust.getCustomername()+" Do you wnat to place your order..Y for yes, N for No:");
            ch = s.next().charAt(0);
            if(ch=='y' || ch=='Y')
            {
                newcust.placeOrder(poq);
